require 'ar_repository/models/crafter'
...
module ArRepository
  class CrafterRepository
    ...
    def create(attributes = {})
      begin
        crafter = model_class.new(attributes)
        crafter.save!
        crafter
    ...
    def where_status(query_string)
      if query_string == "Other"
        model_class.where("status != 'Has Apprentice' AND status != 'Seeking Apprentice' AND status != 'New Crafter' OR status IS NULL")
      else
