...
describe NotePresenter do
  ...
  let(:note) { Footprints::Repository.notes.create(:crafter_id => 1, :applicant_id => 1, :body => "test", :updated_at => given_time) }
  ...
  describe "#created_by?" do
    it "returns true if the user created the note" do
      user = double("user", :crafter_id => 1)
      expect(described_class.new(note).created_by?(user)).to be_true
    end

    it "returns true if the user created the note" do
      user = double("user", :crafter_id => 2)
